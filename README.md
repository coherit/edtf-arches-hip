...
||YYYY-MM-DD/open|1934-02-03/open |Open is used when something has not yet ended -- this can be very useful, for instance, in the classification or designation sections to indicate current usage or designations in effect.|
||YYYY~/YYYY-DD ... |1879~/1965-02|You can use any combination of approximate (~) and uncertain (?) in extended intervals.|
...
## Installation
*Ideally, this project is implemented at installation time because it requires changes to the resource graphs, specifically redefining entity types. Midstream deployment has not been tested.*

Because each Arches project has different data needs, these instructions will guide you through how to implement EDTF on any date field of your choosing. You should use this project as a working reference or demonstration of the feature (rather than installing as is).  
...
This repository deploys EDTF on the following entities as examples: 
- FROM_DATE.E49
- TO_DATE.E49
...
Find the corresponding entities in your `projects/my_hip_app/my_hip_app/source_data/*.E*_nodes.csv` files (this project's directory structure uses "archesdev" in place of my_hip_app) and edit "dates" to "strings" at the end of each corresponding line. 

It helps to understand the CIDOC CRM and the Arches resource graphs to understand which files and lines you will need to edit. 

For instance, if you wanted to change START_DATE_OF_EXISTENCE.E49 to EDTF, you would have to make these edits across all the concept files that define the use of that entity. In this demonstration, it has been changed in the following files and lines:
...
Changes have been made to the form's input field to remove the Bootstrap Datepicker and to edit the alert message that displays when someone enters data in an unsupported format.
...
## Extending Further
EDTF stands for "Extended Date/Time Format"; although this project by default only supports Date formats, it would likely be trivial to add datetime support. 

You would want to start by examining the validateEdtfy function in branch-list.js and edit Line 83, where the moment.js formats are defined as an array.
